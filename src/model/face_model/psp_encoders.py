...
from src.models.encoders.helpers import (
    ...
    bottleneck_IR,
    bottleneck_IR_SE,
    bottleneck_IR_SE_Ours,
    get_block,
    get_blocks,
)
from src.models.stylegan2.model import EqualConv2d, EqualLinear
...
class FSEncoder_PSP(Module):
    def __init__(self, mode="ir_se", opts=None):
        ...
        self.n_styles = 11
        self.input_layer = Sequential(
            Conv2d(3, 64, (3, 3), 1, 1, bias=False), InstanceNorm2d(64), PReLU(64)
        )
        modules = []
        for block in blocks:
            for bottleneck in block:
        ...
        for i in range(b_size):  # for each sample
            for j in range(s_size):  # for each seg cls
                ...
                if component_mask_area > 0:
                    codes_component_feature = (
                        style_feats[i]
                        .masked_select(segmap.bool()[i, j])
                        .reshape(f_size, component_mask_area)
                        .mean(1)
                    )
